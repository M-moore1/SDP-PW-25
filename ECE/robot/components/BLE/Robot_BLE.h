...
typedef struct {
    ...
    bool          notify_enabled;
    bool          authenticated;        // true once BLE auth handshake complete
    esp_bd_addr_t peer_bda;             // peer address, used for bond management
    uint8_t       rx_buf[200];
    int           rx_idx;
    uint8_t       data_mode;
} device_conn_t;
...
extern volatile bool ble_congested;
...
void robot_ble_init();
void ble_apply_security_params();   // call after security_flag changes
