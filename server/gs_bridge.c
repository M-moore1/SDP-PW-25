...
static int uart_send_encrypt(int uart_fd, char *encrypted_json_str)
{
    if (!encrypted_json_str)
        return -1;

    size_t len = strlen(encrypted_json_str);

    // Allocate buffer: original + '\r'
    char *tx_buf = malloc(len + 2);
    if (!tx_buf)
        return -1;

    memcpy(tx_buf, encrypted_json_str, len);
    tx_buf[len] = '\r';
    tx_buf[len + 1] = '\0';

    ssize_t total = 0;
    ssize_t to_write = len + 1;   // include '\r', exclude '\0'

    printf("%s", tx_buf); // Debug: print the encrypted string being sent

    while (total < to_write) {
        ssize_t n = write(uart_fd, tx_buf + total, to_write - total);
...
static int handle_node_json(int uart_fd, int uds_fd, const char *json_str) {
  ...
  cJSON *security = cJSON_GetObjectItemCaseSensitive(root, "security"); // Get "type" field
  if (!cJSON_IsString(security) || !security->valuestring) {     // Ensure it's a string
    cJSON_Delete(root);                                  // Free parsed JSON object
    uds_send_json(uds_fd, "{\"type\":\"ERR\",\"msg\":\"missing type\"}");
    return -1;                                           // Error
  }
  const char *s = security->valuestring; 
  ...
  if (strcmp(s, "F") == 0){
    ...
    uart_send_encrypt(uart_fd, json_str);
